...
func TestEnvironmentVariables(t *testing.T) {
	...
	require.Equal(t, branding.ConfigEnv("NAME"), env.ConfigVariable("NAME"))
}
...
func TestGetConfigVariablePrefersInstantpackPrefix(t *testing.T) {
	env := NewEnvironment(&map[string]string{
		"INSTANTPACK_PACKAGES": "bun@1.2",
		"RAILPACK_PACKAGES":    "node@22",
	})

	value, configName := env.GetConfigVariable("PACKAGES")
	require.Equal(t, "bun@1.2", value)
	require.Equal(t, branding.ConfigEnv("PACKAGES"), configName)
}
...
func TestIsConfigVariableTruthyInstantpackPrefix(t *testing.T) {
	env := NewEnvironment(&map[string]string{
		"INSTANTPACK_BUN_NO_FROZEN_LOCKFILE": "true",
	})
	require.True(t, env.IsConfigVariableTruthy("BUN_NO_FROZEN_LOCKFILE"))
}
