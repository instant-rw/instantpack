...
func TestGetImageEnvIncludesBuiltAt(t *testing.T) {
	...
	for _, variable := range env {
		if value, ok := strings.CutPrefix(variable, "RAILPACK_BUILT_AT="); ok {
			railpackBuiltAt = value
		}
		if value, ok := strings.CutPrefix(variable, "INSTANTPACK_BUILT_AT="); ok {
			instantpackBuiltAt = value
		}
	}
	require.Equal(t, railpackBuiltAt, instantpackBuiltAt)
	timestamp, err := strconv.ParseInt(railpackBuiltAt, 10, 64)
